...
|---|---|---|---|
| Real-time cockpit weather | Mostly covered for prototype | `frontend/src/pages/ExplorerPage.tsx` and `frontend/src/pages/WeatherPage.tsx` show weather/PIREP guidance, latest deltas, affected missions, freshness, time-to-guidance metrics, source-family drilldown chips, and map features extracted from coordinate-bearing SIGMET/AIRMET/CWAP-style text with validity timing and movement vectors when present. Weather table/guidance selection now drives map selection for matching weather features. `LiveAviationWeatherAdapter` can poll NOAA/AWC aviation weather when explicitly enabled, while `/api/weather/patterns`, `/api/weather/events`, and `/api/weather/patterns/features` expose deterministic, source-cited weather patterns and event groups. `frontend/src/pages/MissionPage.tsx` surfaces per-mission weather verdicts. `frontend/src/pages/PilotBriefPage.tsx` provides a printable/read-only handoff brief. | No cockpit-certified display, no per-tail avionics integration, and live polling remains configuration-gated rather than enabled by default. |
...
| Pilot decision support | Mostly covered for prototype | `DecisionPage` exposes operational action, trace, blocking constraints, typed/clickable source refs, audit envelope, replay, map, route predictions, and avoidance candidates. Explorer, Mission, Weather, and Pilot Brief pages now show verdicts, deltas, affected missions, and guidance freshness. The pilot brief groups source drivers by family, prints them in the handoff brief, and shows route-impact source chips so a pilot/controller can see exactly whether guidance came from NOTAM, PIREP, weather, CARF/ALTRV, or USNS traffic. `org.tash.extensions.agentic` adds deterministic semi-autonomous agents for weather impact, mission risk, reroute analysis, coordination drafts, pilot briefs, data-quality scans, and replay audits; the shell-level Agentic Ops panel displays cited findings, typed what-changed deltas, tasks, recommendations, retained run history, task acknowledgments, structured trace Q&A answers, NextGen operating-loop status, audit hashes, and model-ready reasoning-envelope prompt/draft hashes. Agent run/task history now sits behind an `AgentRunStore` seam with in-memory and configurable JSON-file implementations plus searchable list/detail/transition endpoints for audit drilldown by mission, reservation, decision, agent type, source family, task priority, and task route. `OperationalDeltaService` formalizes replay-to-replay and mission-fallback deltas; `ConfigurableLlmReasoningProvider` is disabled by default but gives the future model path a safety-gated provider seam; `ScenarioFixtureGenerator` and calibration fixture loaders provide deterministic regression/corpus growth seams. `ProductReplayCorpusTest`, `ProductApiResourceTest`, and `AgenticOperationsServiceTest` verify the local product replay path across mission verdict, route impact, PIREP relevance, NOTAM-as-constraint source handling, coordination draft, pilot brief, affected missions, metrics, persisted feed artifacts, cited agent output, restart-safe agent history, task transitions, policy enforcement, reasoning-envelope safety rules, trace-answer citations, delta comparison, scenario generation, calibration fixture loading, and auditable decisions. Playwright E2E smoke coverage exercises the Agentic Ops panel from login through run-all, trace Q&A, task acknowledgment, coordination draft, and pilot brief. | Needs larger real-world corpus, calibrated confidence models, operational replay validation against historical outcomes, and real live model/feed integrations once citation/evaluation gates are satisfied. |
...
- current RVR or RVR-equipment status,
...
- Weather-map extraction for common compact aviation coordinates, decimal lat/lon pairs, bounded-by/from-to polygons, validity, altitude, radius, line-corridor, and movement tokens in SIGMET/AIRMET/CWAP-style text, with stable geometry intent metadata and selected map features linking back to their source artifact; non-geometric METAR/TAF products remain station/time guidance artifacts instead of fake shapes.
- Configuration-gated live NOAA/AWC polling plus deterministic weather-pattern APIs for pattern lists, event groups, GeoJSON feature output, and route/corridor sampling. Live mode is off by default, and offline tests use retained fixtures/mocks.
...
- Optional model reasoning is now a config-gated seam, not a default dependency. The local-test mode proves cited provider output can be merged and evaluated without calling an external service; HTTP mode is available only when explicitly configured.
- Scenario generation and calibration loaders are fixture-backed. They support repeatable regression growth and scoring experiments, but they do not replace authoritative FAA/NWS historical validation.
