...
class CarfArtifactRegressionTest {
    ...
    void parsesPriorityZeroNamedFixArtifacts() throws Exception {
        assertParsesReservations(download("tworessies/MOVG HDON CON ONE OF TWO.txt"));
        assertParsesReservations(download("tworessies/MOVG HDON CON TWO OF TWO.txt"));
        assertParsesReservations(download("runtheseandseewhyconflict/FLIGHT1-NOFYI.txt"));
        assertParsesReservations(download("runtheseandseewhyconflict/FLIGHT2-NOFYI.txt"));
        assertParsesReservations(download("attachments/HEADON ONE.txt"));
        assertParsesReservations(download("attachments/HEADON TWO.txt"));
    }

    @Test
    void ptr045HeadOnNoClimbVariantIsVerticallySeparated() throws Exception {
        String ptr = String.join("\n", new org.tash.extensions.evaluation.LegacyArtifactTextExtractor()
                .printableStrings(download("attachments/PTR 045-B-1.doc"), 8));
        assertTrue(ptr.contains("Shows conflict where none should exist"));
        assertTrue(ptr.contains("no conflict detected"));

        CarfRouteMessage first = parser.parse(read(download("attachments/HEADON ONE.txt")));
        CarfRouteMessage second = parser.parse(read(download("attachments/HEADON TWO.txt")));
    ...
    void ptr002OppositeDirectionLongitudinalSeparationHasNoFalseConflict() throws Exception {
        String ptr = textutil(download("PTR 002-R02-B-1.doc"));
        assertTrue(ptr.contains("Opposite direction longitudinal separation"));
        assertTrue(ptr.contains("Expected result is no conflict"));

        CarfRouteMessage first = parser.parse(read(download("ptr-02-A.txt")));
        CarfRouteMessage second = parser.parse(read(download("ptr-02-B.txt")));
    ...
    void parsesNavaidAndRouteControlArtifacts() throws Exception {
        CarfRouteMessage ace = parser.parse(read(download("navaids/ace34l1.txt")));
        CarfRouteMessage ace70 = parser.parse(read(download("navaids/ACE70L2.txt")));
    ...
    void parsesMrkociLateralRadialDmeRegressionPair() throws Exception {
        Map<String, String> entries = new LegacyZipArtifactExtractor().textEntries(download("mrkoci.zip"));
        CarfRouteMessage first = parser.parse(entries.get("mrkoci/JR-TEST-LAT1A.txt"));
        CarfRouteMessage second = parser.parse(entries.get("mrkoci/JR-TEST-LAT1B.txt"));
    ...
    void headOnArtifactsProduceExplainableConflicts() throws Exception {
        CarfRouteMessage first = parser.parse(read(download("runtheseandseewhyconflict/FLIGHT1-NOFYI.txt")));
        CarfRouteMessage second = parser.parse(read(download("runtheseandseewhyconflict/FLIGHT2-NOFYI.txt")));
    ...
    void conflictTooShortListingsCanBeFilteredByMinimumDuration() throws Exception {
        String listing = read(download("conflicttoshort/90-91conflictslisting.txt"));
    ...
    void parsesAdmissionSecondsAndMitoVariants() throws Exception {
        CarfRouteMessage mito = parser.parse(read(download("tworessies/MOVG HDON CON ONE OF TWO.txt")));
        CarfRouteMessage seconds = parser.parse(read(download("tworessies/MOVG HDON CON TWO OF TWO.txt")));
    ...
    void parsesTimingTextFixturesAndClimbAltitudeProfile() throws Exception {
        CarfRouteMessage baseline = parser.parse(read(download("texts/timing test1.txt")));
        CarfRouteMessage climb = parser.parse(read(download("texts/timing test 2.txt")));
    ...
    void messagesZipTimingPairReproducesSameYearClimbConflicts() throws Exception {
        Map<String, String> entries = new LegacyZipArtifactExtractor().textEntries(download("messages.zip"));
    ...
    private String read(Path path) throws Exception {
        assertTrue(Files.exists(path), "Missing artifact: " + path);
        return new String(Files.readAllBytes(path), "UTF-8");
    }

    private String textutil(Path path) throws Exception {
        Process process = new ProcessBuilder("textutil", "-convert", "txt", "-stdout", path.toString())
        ...
        assertEquals(0, code, "textutil failed for " + path);
